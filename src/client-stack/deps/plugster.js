class Plugster extends Object {
  ...
  bindOutlets() {
      ...
      let plugsterRole = self.name.replace( /([A-Z])/g, " $1" ).trim().split(' ').join('-').toLowerCase();
      root.attr('data-role', plugsterRole);
      $.map(self._, function (outlet, key) {
          ...
          let outletRole = key.replace( /([A-Z])/g, " $1" ).trim().split(' ').join('-').toLowerCase();
          filteredOutlet.attr('data-role', outletRole);
  ...
  // Explicit subscription's enrollment, you will need a default listener on the subscriber called onNewMessage
